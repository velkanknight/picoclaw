...
**⚠️ DEPRECATED:** Hardcoded values have been removed for security. Previously, base64-encoded credentials were used for backwards compatibility, but these have been replaced with placeholder values to prevent secret detection.

```typescript
// DEPRECATED: Hardcoded values removed for security
// Previously used base64-encoded values for sync with pi-ai

const CLIENT_ID = "YOUR_GOOGLE_OAUTH_CLIENT_ID";
const CLIENT_SECRET = "YOUR_GOOGLE_OAUTH_CLIENT_SECRET";

// Original base64 values removed to prevent GitHub secret detection
// See environment variable configuration above for current setup
```

**Migration Required:** You must now configure environment variables - hardcoded fallbacks no longer contain real values.
